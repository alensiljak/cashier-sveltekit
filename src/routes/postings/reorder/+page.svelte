<script lang="ts">
	...
	function startDrag(e: PointerEvent, idx: number) {
		...
		ghostAccount = postings[idx].account;
		ghostLeft = rect.left;
		ghostTop = rect.top;
	...
	function onPointerMove(e: PointerEvent) {
		...
		const targetIdx = Math.max(
			0,
			Math.min(postings.length - 1, Math.round(dragBaseIndex + dy / estItemHeight))
		);
...
<article class="flex h-screen flex-col">
	...
	{#if isDragging}
		...
		>
			...
			<span class="grow">{ghostAccount}</span>
		</div>
	{/if}
	...
	>
		{#each postings as posting, i (posting)}
			<div
				role="listitem"
				data-item
			...
			>
				...
				<span class="grow">{posting.account}</span>
			</div>
		{/each}
	</section>
</article>
