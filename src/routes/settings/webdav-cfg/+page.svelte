		...
		<div class="mx-auto max-w-2xl space-y-4">
			...
			<div class="card bg-base-200 shadow-xl">
				<div class="card-body p-4">
					...
					<p class="text-sm opacity-70">
						List the files in the configured directory to verify connectivity and credentials.
					</p>
					<button
						class="btn btn-secondary mt-2 self-start"
										...
										{#each remoteFiles as f}
											<tr>
												...
												<td class="text-right text-xs tabular-nums"
													>{f.size != null ? f.size.toLocaleString() + ' B' : '—'}</td
												>
												<td class="text-xs"
													>{f.lastModified ? f.lastModified.toLocaleString() : '—'}</td
												>
											</tr>
										{/each}
									</tbody>
			...
			{#if corsError}
				<div class="alert alert-warning">
					...
					<div>
						...
						<p class="text-sm">
							The server blocked the request from a browser origin. In NextCloud you may need to add
							the app's origin to <code>config.php</code> under <code>cors.allowed-origins</code>,
							or use an app password with a CORS-enabled proxy.
						</p>
					</div>
				</div>
			{/if}
			...
			<details class="card bg-base-200 shadow-xl" bind:open={uploadOpen}>
				...
				<div class="card-body p-4 pt-0">
					...
					<p class="text-sm opacity-70 mt-2">
						Paste or type the content to send to the remote file.
					</p>
					...
					<div class="form-control mt-2">
						...
						<textarea
							...
							class="textarea textarea-bordered h-40 font-mono text-xs"></textarea>
					</div>

					<button class="btn btn-primary mt-3" onclick={upload} disabled={isUploading}>
						{#if isUploading}
							<span class="loading loading-spinner loading-sm"></span>
						{/if}
						Upload file
			...
			<details class="card bg-base-200 shadow-xl" bind:open={downloadOpen}>
				...
				<div class="card-body p-4 pt-0">
					...
					<button class="btn btn-primary mt-3" onclick={download} disabled={isDownloading}>
						{#if isDownloading}
							<span class="loading loading-spinner loading-sm"></span>
						{/if}
					...
					{#if readContent}
						<div class="form-control mt-3">
							...
							<textarea
								...
								readonly
								class="textarea textarea-bordered h-52 font-mono text-xs"></textarea>
						</div>
					{/if}
				</div>
			</details>
					...
					<ul class="list-disc list-inside space-y-1 text-sm opacity-80">
						<li>
							NextCloud WebDAV URL format: <code class="text-xs"
								>https://&lt;host&gt;/remote.php/dav/files/&lt;user&gt;/&lt;path&gt;/</code
							>
						</li>
						<li>
							Use an <strong>App Password</strong> (Settings → Security) rather than your account password.
						</li>
						<li>
