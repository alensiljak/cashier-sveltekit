<script lang="ts">
	...
	const get_terrain = (terrains: HTMLElement[]) => {
	...
	/** @param {number} time */
	const request_scroll_with_drag = (time: number) => {
		if (request_id) return;
		request_id = requestAnimationFrame(() => scroll_with_drag(time));
	};

	/** @param {number} time */
	const scroll_with_drag = (time: number) => {
		request_id = 0;
		let xs = container;
		for (;;) {
			if (xs === document.body) {
		...
		if (
			drag_rect.left >= x_rect.left &&
			drag_rect.right <= x_rect.right &&
			drag_rect.top >= y_rect.top &&
			drag_rect.bottom <= y_rect.bottom
		) {
			return;
		}
		const now = Date.now();
