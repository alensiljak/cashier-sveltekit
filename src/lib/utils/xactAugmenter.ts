...
export class XactAugmenter {
	 ...
	 */
	static calculateEmptyPostingAmounts(transactions: Xact[]): Xact[] {
		// iterate
	...
	static calculateXactAmount(xact: Xact): Money {
		...
		const postings = xact.postings.filter(
			(posting) =>
				posting.account.startsWith('Assets:') || posting.account.startsWith('Liabilities:')
		);
		...
		if (postings.length === 0) {
			...
			if (
				postings.filter((posting) => posting.account.startsWith('Assets:')).length > 0 &&
				postings.filter((posting) => posting.account.startsWith('Liabilities:')).length > 0
			) {
				// Take the sign from the Asset posting
				const assetPostings = postings.filter((posting) => posting.account.startsWith('Assets:'));
