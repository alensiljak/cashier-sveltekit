...
- [x] Wire per-file diff preview using the shared diff util (`buildDiffLines`, single-file modal); generalized during manual testing so the "Preview" button appears for every non-unchanged row (was gated on both sides having the file, hiding it for new/deleted-file rows) and the modal now diffs local content against what the row's _currently selected_ action would actually produce — remote content for pull (or a pending conflict, previewed as-if-pulled), unchanged local content for skip — with a banner stating the resulting effect
- [x] Wire the conflict-row "Verify" action: `hashFile(path)` on both sources; match upserts the baseline from _both_ the current local and remote entries and clears the row's override; mismatch keeps it a conflict. Also offered on `local-newer` rows with a remote counterpart (nothing to verify for a purely local-only file), for the same "differs only by metadata" case. A bulk "Verify all"/"Verify conflicts" button covers a whole group in one call (grouped-view Conflicts header, bottom Apply bar for unresolved conflicts)
- [x] Split `PeerSyncBaseline` into independent local/remote metadata halves (`localSize/localModified` + `remoteSize/remoteModified` instead of one shared `size/lastModified`) — two devices' filesystems assign different mtimes to byte-identical content, so a single shared baseline value could never match both sides at once. This was causing a hash-verified match (or even a plain pull) to immediately re-show as `remote-newer`/conflict on the next scan; `syncDiff.ts` now compares each side only against its own baseline half. `applyChanges` also re-scans OPFS after writing so the baseline's local half is the file's _actual_ post-write metadata, not a guess
- [x] Per-file preview modal: prev/next arrows (disabled at the ends) scroll between contiguous added/removed hunks; navigation resets whenever the previewed file or its selected pull/skip action changes
- [x] Selected "Skip" toggle button uses `btn-neutral` instead of `btn-primary` — this theme's primary color reads as teal/green, which made Skip look like an affirmative/success action instead of a no-op
- [x] `PeerPresence`: a page refresh/close now sends a best-effort `room.leave()` on the `pagehide` event (relay-propagated, near-instant) instead of relying solely on WebRTC's own disconnect detection (which can take tens of seconds to minutes and was leaving the other peer's UI stuck showing a since-refreshed peer as online). A 5s `getPeers()` poll also evicts any peer whose `RTCPeerConnection.connectionState` has gone `disconnected`/`failed`/`closed`, self-healing cases where even the leave signal is missed (crash, network drop)
...
- [x] Manual two-browser-instance smoke test: pull direction, conflict row, hash-verify match and mismatch, all three view-mode variants
